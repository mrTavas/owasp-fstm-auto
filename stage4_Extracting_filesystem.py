...
def Stage4_Extracting_filesystem(firmware, filesystemType):
    ...
    # Extract error   
        ...
        subprocess.check_call('binwalk ' + firmware + 
        ' | grep -i filesystem | awk \'{print $2}\' >> config.conf', shell=True)
        ...
        subprocess.check_call('dd if=' + firmware + 
        ' bs=1 skip=$((' + addressFilesystem + ')) of=dir.' 
        + filesystemType, shell=True)
